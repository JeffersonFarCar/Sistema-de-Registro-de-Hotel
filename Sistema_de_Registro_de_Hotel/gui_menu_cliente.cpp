...
void Gui_Menu_Cliente::mostrarDatos()
{
    ...
    QString consulta="SELECT nombre, apellido, dni, idpersona, email, ciudadania FROM personas INNER JOIN clientes WHERE personas.idpersona = clientes.id_persona";
...
void Gui_Menu_Cliente::on_lineEdit_buscar_textChanged(const QString &arg1)
{
    ...
    QString consulta="SELECT nombre, apellido, dni, idpersona, email, ciudadania FROM "
                     "personas INNER JOIN clientes WHERE idpersona = id_persona AND nombre LIKE '"+arg1+"%'";
